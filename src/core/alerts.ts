import type { DateISO, Minutes, Settings } from './types';
import type { Pulse } from './engine';
import { formatClockish, formatDuration, formatSigned, minutesOfDay, parseHHMM } from './time';
...
export function snooze(memory: AlertMemory, kind: AlertKind, minutes: Minutes, now: number): AlertMemory {
  return {
    ...memory,
...
function enabled(kind: AlertKind, settings: Settings): boolean {
  ...
  switch (kind) {
    case 'DAY_START': return n.dayStart;
    case 'LUNCH_START': return n.lunchStart;
    case 'LUNCH_END': return n.lunchEnd;
    case 'DAY_END': return n.dayEnd;
