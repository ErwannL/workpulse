...
import { computeDay, plannedMinutes } from './day';
import { DEFAULT_SETTINGS } from './settings';
import { atTimeOn } from './time';
import { entry, fullDay, workDay } from './testing';
...
describe('computeDay', () => {
  ...
  it('signale les séquences incohérentes sans planter', () => {
    const r = computeDay(D, [entry(D, 'BREAK_END', '09:00'), entry(D, 'CLOCK_OUT', '10:00')], at('11:00'));
...
describe('plannedMinutes', () => {
  ...
  it('est nul un jour férié, sauf si le jour est travaillé', () => {
    ...
    expect(plannedMinutes('2026-07-14', workDay('2026-07-14', { status: 'HOLIDAY', worksOnHoliday: true }), s)).toBe(420);
  });
